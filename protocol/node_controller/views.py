...
class NodeControllerSet(ViewSet):
    ...
    def refresh_protocol(self, request, *args, **kwargs):
        ...
        if protocol_name is None:
            return response_data(code=ErrorCode.E_PARAM_ERROR, data="The protocol name cannot be empty")
        ...
        if protocol_name not in protocol_dict.keys():
            return response_data(code=ErrorCode.E_PARAM_ERROR, data='he protocol name does not exist. Please choose one of the following: strict-uRPF,\
        ...
        if returncode != 0:
            return response_data(ErrorCode.E_SERVER, data="Docker command execution failed")
