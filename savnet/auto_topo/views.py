    ...
    def get(self, request, *args, **kwargs):
        ...
        for node in topo_list:
            ...
            for net_interface_name, net_interface_info in net_interface.items():
                ...
                if IP_Addr is None:
                    node["net_interface"] = "10.0." + str(net_seg) + ".1"
                    for peer_node in topo_list:
                        net_interface_str = net_interface_name.split("_")
                        if peer_node["router_name"] != net_interface_str[1].upper():
                            continue
                        peer_net_interface = net_interface_str[1].upper() + "_" + net_interface_str[0].upper()
                        if peer_node["net_interface"].get(net_interface_str) is not None:
                            break
                        else:
                            peer_node["peer_net_interface"] = "10.0." + str(net_seg) + ".2"
                            break
                    net_seg += 1
        # 产生bird的配置文件
        # (1) bird.config
